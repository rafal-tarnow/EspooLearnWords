...
template class KikoBrickLib<J0001Client>;
...
template <typename T>
void KikoBrickLib<T>::saveInConfig(std::string key, float value)
{
    prefs->begin("KIKO_BRICK");
...
template <typename T>
void KikoBrickLib<T>::saveInConfig(std::string key, uint8_t value)
{
    prefs->begin("KIKO_BRICK");
...
template <typename T>
uint8_t KikoBrickLib<T>::readFromConfig(std::string key, uint8_t defaultValue)
{
    prefs->begin("KIKO_BRICK");
    uint8_t value = prefs->getUChar(key.c_str(), defaultValue);
    prefs->end();
    return value;
}

template <typename T>
float KikoBrickLib<T>::readFromConfig(std::string key, float defaultValue)
{
    prefs->begin("KIKO_BRICK");
