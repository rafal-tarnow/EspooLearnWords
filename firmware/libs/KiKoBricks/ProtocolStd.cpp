...
float ProtocolStd::popFloat(std::deque<uint8_t> &frame)
{
    ...
    uint8_t byte1 = frame.front(); frame.pop_front();
    uint8_t byte2 = frame.front(); frame.pop_front();
    uint8_t byte3 = frame.front(); frame.pop_front();
    uint8_t byte4 = frame.front(); frame.pop_front();
    ...
    float result = *reinterpret_cast<const float*>(&value);
    return result;
}
